...
class Transaction_out extends CI_Controller {
	...
	public function insert() {
		...
		$data = array();
		if($id==null){
			$res = $this->db->query("SELECT * FROM master_transaction_out WHERE tgl_terpakai='$tgl_terpakai' AND kode_part='$kode_part'");
			if($res->num_rows() > 0) {
	...
	public function get_data($merk) {
		...
		$query = "SELECT *, a.id as ids, c.idatm as pid FROM master_transaction_out a 
