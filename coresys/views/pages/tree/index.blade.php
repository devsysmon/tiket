											...
											@foreach ($keys as $row)
												<li style="display:none">
													<span><i class="fa fa-lg fa-plus-circle"></i> 
														{{ explode('\\', $row['class'])[7] }}
													</span> ({{count($row['data'])}}) | <a class="btn btn-info btn-xs" href="javascript:void(0);" onclick="read('{{ str_replace('\\', '/', $row['class']) }}')">action</a>
													<ul>
														@foreach ($row['data'] as $row)
															<li style="display:none">
																<a class="btn btn-info btn-xs" href="javascript:void(0);" onclick="read2('{{ str_replace('\\', '/', $row) }}')">
																	<span><i class="icon-leaf"></i>{{ explode('\\', $row)[8] }}</span>
																</a>
															</li>
														@endforeach
													</ul>
