...
class Helpdesk extends MY_Controller {
	...
	public function add() {
		return view('pages/helpdesk/add');
	}
	
	public function edit() {
		return view('pages/helpdesk/edit');
	}
	...
	public function insert() {
		...
		if($id==null) {
			...
			$res = $this->db->get('helpdesk');
			$num = $res->num_rows();
			
			if($num>0) {
				$result['status'] = 'exist';
			} else {
				$res = $this->db->insert('helpdesk', $data);
			...
			// } else {
				$this->db->where('id', $id);
				$res = $this->db->update('helpdesk', $data);
	...
	public function get_data() {
        ...
        $employees = $this->db->get("helpdesk");
        $data = array();
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("helpdesk");
        $result = $query->row();
