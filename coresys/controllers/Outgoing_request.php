...
class Outgoing_request extends MY_Controller {
	var $data = array();
	var $data_temp = array();
	...
	public function index() {	
		...
		return view('pages/outgoing_request/index', $this->data);
	}
	...
	public function add() {
		...
		return view('pages/outgoing_request/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("outgoing_request")->row();
	...
	public function insert() {
		...
		$data = array();
		if($id==null){
			$res = $this->db->query("SELECT * FROM outgoing_request WHERE tgl_terima='$tgl_terima' AND kode_part='$kode_part'");
			if($res->num_rows() > 0) {
				...
				$result = $this->db->update('outgoing_request', $data);
			} else {
				$data['tgl_terima'] = $tgl_terima;
				$data['kode_part'] = $kode_part;
				$data['quantity'] = $quantity;
				$result = $this->db->insert('outgoing_request', $data);
				if($result) {
	...
	public function delete($id) {
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('outgoing_request');
	...
	public function get_data($merk) {
		...
		// $query = "SELECT *, a.id as ids FROM outgoing_request a LEFT JOIN master_inventory b ON(a.kode_part=b.id) $where";
		$query = "SELECT *, a.id as ids, a.quantity, count(a.id) as cnt FROM outgoing_request a 
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("outgoing_request");
        $result = $query->row();
