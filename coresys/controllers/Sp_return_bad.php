...
class Sp_return_bad extends MY_Controller {
	var $data = array();
	var $data_temp = array();
	...
	public function index() {	
		...
		return view('pages/sp_return_bad/index', $this->data);
	}
	...
	public function add() {
		...
		return view('pages/sp_return_bad/form', $this->data);
	}
	
	public function edit($id) {
		$this->db->where("id", $id);
		$row = $this->db->get("sp_return_bad")->row();
	...
	public function insert() {
		...
		$data = array();
		if($id==null){
			$res = $this->db->query("SELECT * FROM sp_return_bad WHERE tgl_terima='$tgl_terima' AND kode_part='$kode_part'");
			if($res->num_rows() > 0) {
				...
				$result = $this->db->update('sp_return_bad', $data);
			} else {
				$data['tgl_terima'] = $tgl_terima;
				$data['kode_part'] = $kode_part;
				$data['quantity'] = $quantity;
				$result = $this->db->insert('sp_return_bad', $data);
				if($result) {
	...
	public function delete($id) {
		...
		$this->db->where('id', $id);
        $result = $this->db->delete('sp_return_bad');
	...
	public function get_data($merk) {
		...
		// $query = "SELECT *, a.id as ids FROM sp_return_bad a LEFT JOIN master_inventory b ON(a.kode_part=b.id) $where";
		$query = "SELECT *, a.id as ids, a.quantity, count(a.id) as cnt FROM sp_return_bad a 
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("sp_return_bad");
        $result = $query->row();
